    ...
    add: (id, qty) => {
      ...
      if (cartItem) {
        if (shoppingCart.methods.hasInventory(id, qty + cartItem.qty)) {
          ...
          alert("There is no more inventory");
        }
      } else {
...
document.querySelectorAll('.item .actions .add').forEach(button => {
  button.addEventListener('click', e => {
    ...
    if (item && item.qty-1 > 0) {
      ...
      console.log('There is no more inventory');
    }
  })
})
}
...
function renderShoppingCart(){
  ...
  <div class="item">
    ...
    <div class="qty">${item.qty} und</div>
    <div class="subtotal">
      <button class="add">Subtotal: "${numberTocurrency(item.qty * dbItem.price)}"</button>
    </div>
    <div class="actions">
  ...
  const totalContainer = `
    <div class="total">${numberTocurrency(total)}</div>
  `;
